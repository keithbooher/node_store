...
export default ({ mobile, input, field_class, label, searchButton, autofocus, meta: { error, touched } }) => {
  ...
  // strictly numbers no decimals or whatevs
  return (
    <div className={`formField numberField ${searchButton ? "w-90" : "" }`}>
      <label>{label}</label>
      <input autoFocus={autofocus} onKeyDown={(e) => preventAlpha(e)} type="tel" className={field_class} value={input.value} {...input} style={{ marginBottom: '5px' }} />
